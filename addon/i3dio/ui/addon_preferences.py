...
class I3D_IO_AddonPreferences(AddonPreferences):
    ...
    converter_mode_tabs: EnumProperty(name="Tabs", items=[("MANUAL", "Manual", ""), ("AUTOMATIC", "Automatic", "")],
                                      default="MANUAL")
    ...
    def draw(self, context):
        ...
        match self.converter_mode_tabs:
            case 'MANUAL':
                ...
                if (next((True for addon in addon_utils.modules() if
                          addon.bl_info.get("name") == "GIANTS I3D Exporter Tools"), False)):
    ...
    def execute(self, context):
        for addon in addon_utils.modules():
            if addon.bl_info.get("name") == "GIANTS I3D Exporter Tools":
                bpy.context.preferences.addons['i3dio'].preferences.i3d_converter_path = str(
                    pathlib.PurePath(addon.__file__).parent.joinpath('util/i3dConverter.exe'))
                break
        return {"FINISHED"}
...
class I3D_IO_OT_download_i3d_converter(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not re.search(r'href="index\.php\?logout=true"', request.text):
            self.report({'WARNING'},
                        "Could not login to https://gdn.giants-software.com/index.php with provided credentials!")
            return {'CANCELLED'}
        ...
        result = re.findall(
            r'href="download.php\?downloadId=([0-9]+)">Blender Exporter Plugins v([0-9]+.[0-9]+.[0-9]+)', request.text)
        ...
        try:
            ...
            with zipfile.open('io_export_i3d/util/i3dConverter.exe') as zipped_binary, open(binary_path,
                                                                                            'wb') as saved_binary:
            ...
            self.report({'WARNING'},
                        f"The Community I3D Exporter did not succesfully fetch and install the Giants I3D Converter binary! ({e})")
            return {'CANCELLED'}

        self.report({'INFO'},
                    f"Fetched i3dConverter.exe from version {exporter_version} of the Giants Exporter downloaded from {download_url}")
        return {'FINISHED'}
