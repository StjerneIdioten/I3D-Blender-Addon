...
def write_dds_dx10(filepath: str, arr: np.ndarray) -> None:
    ...
    Args:
        ...
        arr: numpy array with shape (array_size, height, width, 4) and dtype float16.
    """
    if not isinstance(arr, np.ndarray):
        raise TypeError(f"arr must be a numpy.ndarray, got {type(arr)!r}")
    if arr.ndim != 4:
        raise ValueError(f"arr must have shape (array_size, height, width, 4), got {arr.shape!r}")

    array_size, height, width, channels = arr.shape
    if channels != 4:
        raise ValueError(f"Channels must be 4 (RGBA), got {channels}")
    if arr.dtype != np.float16:
        raise ValueError(f"arr must be float16 array, got {arr.dtype}")
    if array_size < 1 or height < 1 or width < 1:
        raise ValueError(f"Invalid DDS dimensions: array_size={array_size} height={height} width={width}")
