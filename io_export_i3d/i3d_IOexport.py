...
class I3D_IOexport(object):
    def __init__(self):
        ...
        self._i3d_exportAnimation = \
            bpy.context.scene.I3D_export.I3D_exportAnimation
        self._i3d_exportShapes = bpy.context.scene.I3D_export.I3D_exportShapes
        self._i3d_exportNurbsCurves = \
            bpy.context.scene.I3D_export.I3D_exportNurbsCurves
        self._i3d_exportLights = bpy.context.scene.I3D_export.I3D_exportLights
        self._i3d_exportCameras = \
            bpy.context.scene.I3D_export.I3D_exportCameras
        self._i3d_exportParticleSystems = \
            bpy.context.scene.I3D_export.I3D_exportParticleSystems
        self._i3d_exportUserAttributes = \
            bpy.context.scene.I3D_export.I3D_exportUserAttributes
        self._i3d_exportNormals = \
            bpy.context.scene.I3D_export.I3D_exportNormals
        self._i3d_exportColors = bpy.context.scene.I3D_export.I3D_exportColors
        self._i3d_exportTexCoords = \
            bpy.context.scene.I3D_export.I3D_exportTexCoords
        self._i3d_exportSkinWeigths = \
            bpy.context.scene.I3D_export.I3D_exportSkinWeigths
        self._i3d_exportVerbose = \
            bpy.context.scene.I3D_export.I3D_exportVerbose
        self._i3d_exportRelativePaths = \
            bpy.context.scene.I3D_export.I3D_exportRelativePaths
        self._i3d_exportApplyModifiers = \
            bpy.context.scene.I3D_export.I3D_exportApplyModifiers
        self._I3D_exportAxisOrientations = \
            bpy.context.scene.I3D_export.I3D_exportAxisOrientations
        self._i3d_exportUseBlenderFileName = \
            bpy.context.scene.I3D_export.I3D_exportUseBlenderFileName
        self._i3d_exportFileLocation = \
            bpy.context.scene.I3D_export.I3D_exportFileLocation
        ...
        self._objectsToExportDict = {}
        # KEY: reference to 'bpy.types.Object'VALUE: nodeID
        self._filesToExportDict = {}
        # KEY: reference to 'bpy.types.ImageTexture' VALUE: fileID
        self._filesToExportCustomDict = {}
        # KEY: (m_custom_shader, m_mat[ m_custom_shader ]) or
        # (m_texture, m_mat[ m_texture ]) VALUE: (fileID, colorProfile or None)
        self._materialsToExportDict = {}
        # KEY: reference to 'bpy.types.Material' VALUE: fileID
        self._materialNone = None  # if (not None) VALUE: fileID
        self._shapesMeshToExportDict = {}
        # KEY: reference to 'bpy_types.Mesh'
        # VALUE: (shapeID, reference to 'bpy.types.Object', string MaterialIDs)
        self._shapesCurveToExportDict = {}
        # KEY: reference to 'bpy.types.Curve' VALUE: shapeID
        self._dynamicsToExportDict = {}
        # KEY: reference to 'bpy.types.Object'
        # VALUE:(nodeID, dynamicsID,
        # reference to 'bpy.types.ParticleSystemModifier')
    ...
    def _makeShapesToExportDicts(self):
        for m_obj in self._objectsToExportDict.keys():
            if ("MESH" == m_obj.type and
                    (m_obj.data not in self._shapesMeshToExportDict)):
                self._shapeID += 1
                self._shapesMeshToExportDict[m_obj.data] = \
                    (self._shapeID, m_obj, '')
            if ("CURVE" == m_obj.type and
                    (m_obj.data not in self._shapesCurveToExportDict)):
                self._shapeID += 1
    ...
    def _makeDynamicsToExportDict(self):
        for m_obj in self._objectsToExportDict.keys():
            if ("MESH" == m_obj.type and
                    (m_obj not in self._dynamicsToExportDict)):
                m_modifier = self._isObjectHasParticleSystem(m_obj)
            if (m_modifier):
                self._dynamicsID += 1
                self._nodeID += 1
                self._dynamicsToExportDict[m_obj] = \
                    (self._nodeID, self._dynamicsID, m_modifier)
    #
    #  =====================================================================================
    #
...
def _xmlWriteFiles(self):
    ...
    for m_file, m_id in self._filesToExportDict.items():
        ...
        if self._i3d_exportRelativePaths:
            ...
            self._xml_files_item.set("filename",
                                     bpy.path.abspath(m_file.image.filepath))
...
def _xmlWriteMaterials(self):
    ...
    if (self._materialNone):
        self._xml_materials_item = xml_ET.SubElement(self._xml_materials,
                                                     "Material")
...
def _xmlWriteMaterial(self, m_mat):
    ...
    if (m_mat.use_transparency and
            ("RAYTRACE" == m_mat.transparency_method)):
        m_alpha_blending = False
        m_refraction_use = True
    # --------------------------------
    # custom data
    # --------------------------------
    m_custom_shader, m_custom_shader_variation, m_custom_texture,\
        m_custom_texture_wrap, m_custom_texture_colorProfile,\
        m_custom_parameter = self._checkMaterialCustomData(m_mat)
    # --------------------------------
    # write to xml
    # --------------------------------
    self._xml_materials_item = xml_ET.SubElement(self._xml_materials,
    ...
    if (m_emission_use):
        if (m_map_emission):
            self._xml_materials_item_texture = \
                xml_ET.SubElement(self._xml_materials_item, "Emissivemap")
            self._xml_materials_item_texture.set(
                "fileId", "%d" % self._filesToExportDict[m_map_emission])
            if (not self._isTextureWrapped(m_map_emission)):
                self._xml_materials_item_texture.set("wrap", "false")
        else:
            self._xml_materials_item.set(
                "emissiveColor", "%.6f %.6f %.6f 1" % (m_emission_color.r,
                                                       m_emission_color.g,
                                                       m_emission_color.b))
    else:
        if (m_map_diffuse):
            self._xml_materials_item_texture = xml_ET.SubElement(
                self._xml_materials_item, "Texture")
            self._xml_materials_item_texture.set("fileId", "%d" %
                                                 self._filesToExportDict
                                                 [m_map_diffuse])
        if (not self._isTextureWrapped(m_map_diffuse)):
            self._xml_materials_item_texture.set("wrap", "false")
        else:
            self._xml_materials_item.set(
                        "diffuseColor",
                        "%.6f %.6f %.6f 1" % (m_diffuse_color.r,
                                              m_diffuse_color.g,
                                              m_diffuse_color.b))
        # --------------------------------
        ...
        if (m_map_specular):
            self._xml_materials_item_texture = xml_ET.SubElement(
                self._xml_materials_item, "Glossmap")
        self._xml_materials_item_texture.set(
            "fileId", "%d" % self._filesToExportDict[m_map_specular])
        if (not self._isTextureWrapped(m_map_specular)):
            self._xml_materials_item_texture.set("wrap", "false")
            self._xml_materials_item.set("specularColor", "1 1 1")
        else:
            self._xml_materials_item.set(
                "specularColor", "%.6f %.6f %.6f" % (m_specular_color.r,
                                                     m_specular_color.g,
                                                     m_specular_color.b))
        # --------------------------------
        if (m_map_normal):
            self._xml_materials_item_texture = xml_ET.SubElement(
                self._xml_materials_item, "Normalmap")
            self._xml_materials_item_texture.set(
                "fileId", "%d" % self._filesToExportDict[m_map_normal])
            if (not self._isTextureWrapped(m_map_normal)):
                self._xml_materials_item_texture.set("wrap", "false")
        # --------------------------------
        if (m_reflection_use):
            self._xml_materials_item_texture = xml_ET.SubElement(
                self._xml_materials_item, "Reflectionmap")
            if (m_map_reflection):
                self._xml_materials_item_texture.set(
                    "fileId", "%d" % self._filesToExportDict[m_map_reflection])
                self._xml_materials_item_texture.set("type", "cube")
                self._xml_materials_item_texture.set("wrap", "false")
            else:
                self._xml_materials_item_texture.set("type", "planar")
            self._xml_materials_item_texture.set(
                "refractiveIndex", "%s" % m_refractive_index)
            self._xml_materials_item_texture.set("bumpScale", "0.1")
        # --------------------------------
        if (m_refraction_use):
            self._xml_materials_item_texture = xml_ET.SubElement(
                self._xml_materials_item, "Refractionmap")
            self._xml_materials_item_texture.set("type", "planar")
        ...
        if m_custom_shader:
            ...
            self._xml_materials_item.set(
                "customShaderId",
                "%s" % self._filesToExportCustomDict[m_key][0])
        if m_custom_shader_variation:
            self._xml_materials_item.set(
                "customShaderVariation",
                "%s" % m_mat[m_custom_shader_variation])
        ...
        if (len(m_custom_texture)):
            for m_texture in m_custom_texture:
                ...
                self._xml_materials_item_texture = xml_ET.SubElement(
                    self._xml_materials_item, "Custommap")
                self._xml_materials_item_texture.set(
                    "name", "%s" % m_textureName)
                self._xml_materials_item_texture.set(
                    "fileId", "%s" % self._filesToExportCustomDict[m_key][0])
                ...
                if (len(m_custom_texture_wrap)):
                    for m_textureWrap in m_custom_texture_wrap:
                        ...
                        if ((m_textureName == m_textureWrapName) and
                            (("false" == m_textureWrapValue) or
                             ("true" == m_textureWrapValue))):
                            self._xml_materials_item_texture.set(
                                "wrap", m_textureWrapValue)

        if (len(m_custom_parameter)):
            for m_parameter in m_custom_parameter:
                self._xml_materials_item_texture = xml_ET.SubElement(
                    self._xml_materials_item, "CustomParameter")
                self._xml_materials_item_texture.set(
                    "name", "%s" % m_parameter.split("_")[1])
                self._xml_materials_item_texture.set(
                    "value", "%s" % m_mat[m_parameter])
    #
    #  =====================================================================================
    #
...
def _xmlWriteScene(self):
    ...
    def _xmlWriteSceneObject_Dynamic(self, m_obj, m_xmlCurrent):
        ...
        m_translation, m_rotation, m_scale = self._getTranslationRotationScale(
            m_obj)
        ...
        if (m_castsShadows):
            m_xmlCurrent.set("castsShadows", "true")
        else:
            m_xmlCurrent.set("castsShadows", "false")
        if (m_receiveShadows):
            m_xmlCurrent.set("receiveShadows", "true")
        else:
            m_xmlCurrent.set("receiveShadows", "false")
        if (m_nonRenderable):
            m_xmlCurrent.set("nonRenderable", "true")
    ...
    def _xmlWriteSceneObject_ShapeMesh(self, m_obj, m_xmlCurrent):
        ...
        if (m_compoundChild):
            m_rigidBody = True
        if (255 != m_obj.I3D_attributes.I3D_collisionMask):
            m_collisionMask = m_obj.I3D_attributes.I3D_collisionMask
        if (m_rigidBody):
            if (0.0 != m_obj.I3D_attributes.I3D_restitution):
                m_restitution = m_obj.I3D_attributes.I3D_restitution
            if (0.5 != m_obj.I3D_attributes.I3D_staticFriction):
                m_staticFriction = m_obj.I3D_attributes.I3D_staticFriction
            if (0.5 != m_obj.I3D_attributes.I3D_dynamicFriction):
                m_dynamicFriction = m_obj.I3D_attributes.I3D_dynamicFriction
            if (0.5 != m_obj.I3D_attributes.I3D_linearDamping):
                m_linearDamping = m_obj.I3D_attributes.I3D_linearDamping
            if (0.5 != m_obj.I3D_attributes.I3D_angularDamping):
                m_angularDamping = m_obj.I3D_attributes.I3D_angularDamping
            if (0.05 != m_obj.I3D_attributes.I3D_skinWidth):
                m_skinWidth = m_obj.I3D_attributes.I3D_skinWidth
            if (1 != m_obj.I3D_attributes.I3D_density):
                m_density = m_obj.I3D_attributes.I3D_density
            if (4 != m_obj.I3D_attributes.I3D_solverIterationCount):
                m_solverIterationCount = \
                    m_obj.I3D_attributes.I3D_solverIterationCount
        m_xmlCurrent.set("shapeId", "%d" % m_shapeID)
        m_xmlCurrent.set("materialIds", m_materialIDs)
        if (m_static):
            m_xmlCurrent.set("static", "true")
        if (m_dynamic):
            m_xmlCurrent.set("dynamic", "true")
        if (m_kinematic):
            m_xmlCurrent.set("kinematic", "true")
        if (m_compound):
            m_xmlCurrent.set("compound", "true")
        if (m_compoundChild):
            m_xmlCurrent.set("compoundChild", "true")
        if (not m_collision):
            m_xmlCurrent.set("collision", "false")
        if (m_trigger):
            m_xmlCurrent.set("trigger", "true")
        if (m_collisionMask):
            m_xmlCurrent.set("collisionMask", "%s" % m_collisionMask)
        if (m_restitution):
            m_xmlCurrent.set("restitution", "%s" % m_restitution)
        if (m_staticFriction):
            m_xmlCurrent.set("staticFriction", "%s" % m_staticFriction)
        if (m_dynamicFriction):
            m_xmlCurrent.set("dynamicFriction", "%s" % m_dynamicFriction)
        if (m_linearDamping):
            m_xmlCurrent.set("linearDamping", "%s" % m_linearDamping)
        if (m_angularDamping):
            m_xmlCurrent.set("angularDamping", "%s" % m_angularDamping)
        if (m_skinWidth):
            m_xmlCurrent.set("skinWidth", "%s" % m_skinWidth)
        if (m_density):
            m_xmlCurrent.set("density", "%s" % m_density)
        if (m_solverIterationCount):
            m_xmlCurrent.set(
                "solverIterationCount", "%s" % m_solverIterationCount)
        if (m_rigidBody and m_ccd):
            m_xmlCurrent.set("ccd", "true")
    ...
    def _xmlWriteSceneObject_Light(self, m_obj, m_xmlCurrent):
        ...
        if (("POINT" == m_obj.data.type) or
                ("SPOT" == m_obj.data.type)):
            if ("INVERSE_LINEAR" == m_obj.data.falloff_type):
                m_decayRate = "1"
            elif ("INVERSE_SQUARE" == m_obj.data.falloff_type):
                m_decayRate = "2"
            m_decayRate = "0"
        m_diffuseColor = "%.3f %.3f %.3f" % m_obj.data.color[:]
        m_specularColor = "%.3f %.3f %.3f" % m_obj.data.color[:]
        if (m_obj.data.use_diffuse):
            m_emitDiffuse = "true"
        else:
            m_emitDiffuse = "false"
        if (m_obj.data.use_specular):
            m_emitSpecular = "true"
        else:
            m_emitSpecular = "false"
    ...
    def _xmlWriteSceneObject_TransformGroup(self, m_obj, m_xmlCurrent):
        ...
        if (m_obj.I3D_attributes.I3D_lod):
            if (m_obj.I3D_attributes.I3D_lod1):
                ...
                if (m_obj.I3D_attributes.I3D_lod2 >
                        m_obj.I3D_attributes.I3D_lod1):
                    m_lodDistance += " %s" % m_obj.I3D_attributes.I3D_lod2
                    if(m_obj.I3D_attributes.I3D_lod3 >
                            m_obj.I3D_attributes.I3D_lod2):
                        m_lodDistance += " %s" % m_obj.I3D_attributes.I3D_lod3
        if (m_lodDistance):
            m_xmlCurrent.set("lodDistance", m_lodDistance)

        if (m_lodDistance):
            m_joint = None
        if (0.010 != m_obj.I3D_attributes.I3D_projDistance):
            m_projDistance = m_obj.I3D_attributes.I3D_projDistance
        if (0.010 != m_obj.I3D_attributes.I3D_projAngle):
            m_projAngle = m_obj.I3D_attributes.I3D_projAngle
        if (100000.0 != m_obj.I3D_attributes.I3D_driveForceLimit):
            m_driveForceLimit = m_obj.I3D_attributes.I3D_driveForceLimit
        if (1.000 != m_obj.I3D_attributes.I3D_driveSpring):
            m_driveSpring = m_obj.I3D_attributes.I3D_driveSpring
        if (0.010 != m_obj.I3D_attributes.I3D_driveDamping):
            m_driveDamping = m_obj.I3D_attributes.I3D_driveDamping
        if (0.000 != m_obj.I3D_attributes.I3D_jointBreakForce):
            m_jointBreakForce = m_obj.I3D_attributes.I3D_jointBreakForce
        if (0.000 != m_obj.I3D_attributes.I3D_jointBreakTorque):
            m_jointBreakTorque = m_obj.I3D_attributes.I3D_jointBreakTorque
        if (m_joint):
            m_xmlCurrent.set("joint", "true")
            if (m_projection):
                m_xmlCurrent.set("projection", "true")
            if (m_xAxisDrive):
                m_xmlCurrent.set("xAxisDrive", "true")
            if (m_yAxisDrive):
                m_xmlCurrent.set("yAxisDrive", "true")
            if (m_zAxisDrive):
                m_xmlCurrent.set("zAxisDrive", "true")
            if (m_drivePos):
                m_xmlCurrent.set("drivePos", "true")
            if (m_breakableJoint):
                m_xmlCurrent.set("breakableJoint", "true")
            if (m_projDistance):
                m_xmlCurrent.set("projDistance", "%s" % m_projDistance)
            if (m_projAngle):
                m_xmlCurrent.set("projAngle", "%s" % m_projAngle)
            if (m_driveForceLimit):
                m_xmlCurrent.set("driveForceLimit", "%s" % m_driveForceLimit)
            if (m_driveSpring):
                m_xmlCurrent.set("driveSpring", "%s" % m_driveSpring)
            if (m_driveDamping):
                m_xmlCurrent.set("driveDamping", "%s" % m_driveDamping)
            if (m_jointBreakForce):
                m_xmlCurrent.set("jointBreakForce", "%s" % m_jointBreakForce)
            if (m_jointBreakTorque):
                m_xmlCurrent.set("jointBreakTorque", "%s" % m_jointBreakTorque)
    ...
    def _xmlWriteVisClipObjmaskLightmask(m_obj, m_xmlCurrent):
        ...
        if (255 != m_obj.I3D_attributes.I3D_objectMask):
            m_objectMask = m_obj.I3D_attributes.I3D_objectMask
        if ("FFFF" != m_obj.I3D_attributes.I3D_lightMask):
            m_lightMask = int(m_obj.I3D_attributes.I3D_lightMask, 16)
        if (m_visibility):
            m_xmlCurrent.set("visibility", "false")
        if (m_clipDistance):
            m_xmlCurrent.set("clipDistance", "%s" % m_clipDistance)
        if (m_objectMask):
            m_xmlCurrent.set("objectMask", "%s" % m_objectMask)
        if (m_lightMask):
            m_xmlCurrent.set("lightMask", "%s" % m_lightMask)
        if (m_decalLayer):
            m_xmlCurrent.set("decalLayer", "%s" % m_decalLayer)
    ...
    def _getTranslationRotationScale(self, m_obj):
        ...
        if ("BAKE_TRANSFORMS" == m_orient):
            ...
            m_matrix = mathutils.Matrix.Rotation(
                math.radians(-90), 4, "X") * m_obj.matrix_local * \
            ...
            if ("CAMERA" == m_obj.type or "LAMP" == m_obj.type):
                m_matrix = m_matrix * mathutils.Matrix.Rotation(
                    math.radians(-90), 4, "X")
            if (m_obj.parent):
                if ("CAMERA" == m_obj.parent.type or
                        "LAMP" == m_obj.parent.type):
                    m_matrix = mathutils.Matrix.Rotation(
    ...
    def _xmlWriteSceneObject(self, m_obj, m_xmlParent):
        ...
        # (only if they are replacement for light or cameras)
        # - Keep Axis mode
        ...
        if ("MESH" == m_obj.type and
            ...
            m_objType = "Shape"
        if ("MESH" == m_obj.type and
                (m_obj in self._dynamicsToExportDict)):
            m_objType = "Dynamic"
        if ("CURVE" == m_obj.type):
            m_objType = "Shape"
        if ("EMPTY" == m_obj.type):
            m_objType = "TransformGroup"
        if ("CAMERA" == m_obj.type):
            m_objType = "Camera"
        if ("LAMP" == m_obj.type):
            m_objType = "Light"
        if (not self._checkObject(m_obj)):
            m_objType = "TransformGroup"
        if (not self._i3d_exportParticleSystems and "Dynamic" == m_objType):
            m_objType = "Shape"
        if (not self._i3d_exportShapes and "MESH" == m_obj.type):
            m_objType = "TransformGroup"
        if (not self._i3d_exportNurbsCurves and "CURVE" == m_obj.type):
            m_objType = "TransformGroup"
        if (not self._i3d_exportCameras and "CAMERA" == m_obj.type):
            m_objType = "TransformGroup"
        if (not self._i3d_exportLights and "LAMP" == m_obj.type):
            m_objType = "TransformGroup"
    ...
    def _xmlWriteShape_Curve(self, m_curve):
        ...
        xml_shapes_nurbsCurve = xml_ET.SubElement(
            self._xml_shapes, "NurbsCurve")
        ...
        if (len(m_curve.splines)):
            ...
            if ("NURBS" == m_spline.type):
                m_points = m_spline.points
            if ("BEZIER" == m_spline.type):
                m_points = m_spline.bezier_points
            if (m_points):
                for m_point in m_points:
                    xml_shapes_nurbsCurve_item = xml_ET.SubElement(
                        xml_shapes_nurbsCurve, "cv")

                    m_pointCoords = m_point.co.xyz[:]
                    if ("BAKE_TRANSFORMS" == self._I3D_exportAxisOrientations):
                        m_pointCoords = (m_pointCoords[0],
                                         m_pointCoords[2],
                                         - m_pointCoords[1])  # x z -y

                    xml_shapes_nurbsCurve_item.set(
                        "c", "%.6f %.6f %.6f" % (m_pointCoords))
        xml_shapes_nurbsCurve.set("form", m_form)

    def _xmlWriteShape_Mesh(self, m_obj, m_meshSource, m_meshID):
        m_meshGen = m_obj.to_mesh(
            bpy.context.scene, self._i3d_exportApplyModifiers, "PREVIEW",
            calc_tessface=False)
        # triangulate mesh before exporting
        self._shapeMeshTriangulate(m_meshGen, self._i3d_exportVerbose)
        if (self._i3d_exportVerbose):
            print("GENERATED MESH IS: %s" % m_meshGen.name)

        m_materialIDs = self._xmlWriteShape_MeshData(
            m_obj, m_meshGen, m_meshID)

        if (self._i3d_exportVerbose):
            print("GENERATED MESH %s WAS REMOVED" % m_meshGen.name)
        ...
        self._shapesMeshToExportDict[m_meshSource] = (m_meshData[0],
                                                      m_meshData[1],
                                                      m_materialIDs)
    ...
    def _xmlWriteShape_MeshData(self, m_obj, m_mesh, m_meshID):
        ...
        if ("BAKE_TRANSFORMS" == self._I3D_exportAxisOrientations):
            m_format = "baked"
        xml_shapes_indTriangleSet = xml_ET.SubElement(
            self._xml_shapes, "IndexedTriangleSet")
        ...
        xml_shapes_indTriangleSet.set(
            "bvCenter", "%.6f %.6f %.6f" % (m_bvCenter.xyz[:]))
        ...
        if (m_needColors):
            ...
            if (m_vertColorsIndexes > 1):
                m_vertColorsIndexes = 1
        else:
            m_vertColorsIndexes = 0

        m_polygonsByMaterialIndex = [[] for
                                     m_i in range(len(m_mesh.materials))]
        if (0 == len(m_polygonsByMaterialIndex)):
            m_polygonsByMaterialIndex.append([])
            # object do not have materials assigned
        for m_polygon in m_mesh.polygons:
            m_polygonsByMaterialIndex[
                m_polygon.material_index].append(m_polygon.index)

        xml_shapes_indTriangleSetVertices = xml_ET.SubElement(
            xml_shapes_indTriangleSet, "Vertices")
        xml_shapes_indTriangleSetTriangles = xml_ET.SubElement(
            xml_shapes_indTriangleSet, "Triangles")
        xml_shapes_indTriangleSetSubsets = xml_ET.SubElement(
            xml_shapes_indTriangleSet, "Subsets")
        if (m_needNormal):
        ...
        if (m_needTexCoords):
            for m_uvlayerIndex in range(m_uvLayersIndexes):
                xml_shapes_indTriangleSetVertices.set(
                    "uv%d" % m_uvlayerIndex, "true")
        ...
        for m_matIndex, m_polygonIndices in enumerate(
                m_polygonsByMaterialIndex):
            ...
            if (len(m_polygonIndices)):
                ...
                if (0 == len(m_mesh.materials)):
                    ...
                    m_materialID = self._getIDbyMaterial(
                        m_mesh.materials[m_matIndex])
                ...
                for m_polygonIndex in m_polygonIndices:
                    ...
                    for m_loopIndex in m_polygonLoopIndices:
                        ...
                        #
                        m_indexData = IndexBufferItem(m_mesh, m_loopIndex,
                                                      m_needNormal,
                                                      m_uvLayersIndexes,
                                                      m_vertColorsIndexes,
                                                      m_format)
                        if (m_indexData not in m_indexBuffer):
                            ...
                            m_counter += 1
                            # save data to xml
                            xml_shapes_indTriangleSetVertices_item = xml_ET.\
                                SubElement(
                                    xml_shapes_indTriangleSetVertices, "v")
                            xml_shapes_indTriangleSetVertices_item.set(
                                "p", "%.6f %.6f %.6f" % m_indexData.
                                m_vertexCoords)
                            if (m_needNormal):
                                xml_shapes_indTriangleSetVertices_item.set(
                                    "n", "%.6f %.6f %.6f" % m_indexData.
                                    m_vertexNormals)
                            if (m_needTexCoords):
                                for m_uvlayerIndex in range(len(
                                        m_indexData.m_vertexUVs)):
                                    xml_shapes_indTriangleSetVertices_item.set(
                                        "t%d" % m_uvlayerIndex,
                                        "%.6f %.6f" % m_indexData.m_vertexUVs[
                                             m_uvlayerIndex])
                            if (m_needColors):
                                for m_vertColorIndex in range(len(
                                        m_indexData.m_vertexColors)):
                                    xml_shapes_indTriangleSetVertices_item.set(
                                        "c", "%.6f %.6f %.6f" % m_indexData.
                                        m_vertexColors[m_vertColorIndex])
                    ...
                    xml_shapes_indTriangleSetTriangles_item = \
                        xml_ET.SubElement(
                            xml_shapes_indTriangleSetTriangles, "t")
                    xml_shapes_indTriangleSetTriangles_item.set(
                        "vi", m_strVI.strip())

                xml_shapes_indTriangleSetSubsets_item = xml_ET.SubElement(
                    xml_shapes_indTriangleSetSubsets, "Subset")
                xml_shapes_indTriangleSetSubsets_item.set(
                    "firstVertex", "%s" % min(m_numVerticesSet))
                xml_shapes_indTriangleSetSubsets_item.set(
                    "numVertices", "%s" % (max(m_numVerticesSet) + 1))
                xml_shapes_indTriangleSetSubsets_item.set(
                    "firstIndex", "%s" % m_firstIndex)
                xml_shapes_indTriangleSetSubsets_item.set(
                    "numIndices", "%s" % m_numIndices)
        xml_shapes_indTriangleSetVertices.set("count",
                                              "%s" % len(m_indexBuffer))
        xml_shapes_indTriangleSetTriangles.set("count",
                                               "%s" % m_trainglesCount)
        xml_shapes_indTriangleSetSubsets.set("count",
                                             "%s" % m_subsetsCount)

        if (self._i3d_exportVerbose):
            m_end_time = time.time()
            print("%s time is %g seconds" % (inspect.stack()[0][3],
    ...
    def _xmlWriteDynamics_ParticleSystem(self, m_obj):
        ...
        xml_dynamics_particleSystem = xml_ET.SubElement(
            self._xml_dynamics, "ParticleSystem")
        ...
        xml_dynamics_particleSystem_gravity = xml_ET.SubElement(
            xml_dynamics_particleSystem, "Gravity")
    ...
    def _xmlWriteUserAttributes_item(self, m_obj):
        m_nodeID = self._objectsToExportDict[m_obj]
        xml_userAttributes_item = xml_ET.SubElement(
            self._xml_userAttributes, "UserAttribute")
        ...
        for m_key in m_obj.keys():
            if (0 == ("%s" % m_key).find("userAttribute_")):
                ...
                xml_userAttributes_item_attr = xml_ET.SubElement(
                    xml_userAttributes_item, "Attribute")
                xml_userAttributes_item_attr.set("name",  m_attributeName)
                xml_userAttributes_item_attr.set("type",  m_attributeType)
                xml_userAttributes_item_attr.set(
                    "value", "%s" % m_attributeValue)
    #
    #  =====================================================================================
    #
    ...
    def _xmlBuild(self):
        ...
        if (self._i3d_exportUserAttributes):
            self._xml_userAttributes = xml_ET.SubElement(
                self._xml_i3d, "UserAttributes")
    ...
    def _getTexturesFromMaterial(self, m_mat):
        ...
        for m_slot in m_mat.texture_slots:
            if ((m_slot) and
                ...
                m_textures[self._checkTextureInSlot(
                    m_mat, m_slot)] = m_slot.texture
        return m_textures
...
def _checkObject(m_obj):
    ...
    if ("MESH" == m_obj.type or
            ...
            ("LAMP" == m_obj.type and
             ("POINT" == m_obj.data.type or
              "SUN" == m_obj.data.type or
              "SPOT" == m_obj.data.type))):
        m_return = True
    return m_return
...
def _checkObjectFiles(self, m_obj):
    ...
    if (m_custom_shader):
        ...
        if ((m_textureName == m_textureCPName) and
                (("sRGB" == m_textureCPValue) or
                 ("linearRGB" == m_textureCPValue))):
        ...
        if (m_key not in self._filesToExportCustomDict):
            self._fileID += 1
            self._filesToExportCustomDict[m_key] = (
                self._fileID, m_colorProfile)
        else:
            if (m_colorProfile and not self._filesToExportCustomDict[
                    m_key][1]):
                self._filesToExportCustomDict[m_key] = (
                    self._filesToExportCustomDict[m_key][0], m_colorProfile)
    ...
    def _checkMaterialCustomData(m_mat):
        ...
        return (m_custom_shader, m_custom_shader_variation, m_custom_texture,
                m_custom_texture_wrap, m_custom_texture_colorProfile,
                m_custom_parameter)
    ...
    def _getUserAttributeType(m_obj, m_key):
        ...
        if ("scriptCallback" == m_attributeType):
            return m_attributeType
        if ("boolean" == m_attributeType):
            return m_attributeType
        if ("float" == m_attributeType):
            return m_attributeType
        if ("integer" == m_attributeType):
            return m_attributeType
        if ("string" == m_attributeType):
            return m_attributeType
        return "string"

    @staticmethod
    def _checkTextureInSlot(m_mat, m_slot):
        if (m_slot.use_map_color_diffuse and m_mat.use_shadeless):
            return "map_emission"
        ...
        if (m_slot.use_map_color_reflection):
            return "map_specular"
        if (m_slot.use_map_mirror and m_mat.raytrace_mirror.use):
            return "map_reflection"
        return None
...
class IndexBufferItem(object):
    def __init__(self, m_mesh, m_index, m_needNormals=True,
                 m_uvLayersIndexes=0, m_vertColorsIndexes=0, m_format=None):

        self.m_format = m_format

        m_indexData = self._getVertexDataByLoopIndex(m_mesh, m_index,
                                                     m_needNormals,
                                                     m_uvLayersIndexes,
                                                     m_vertColorsIndexes)
    ...
    def __hash__(self):
        if (BLENDER_CHECK_VERSION01 <= bpy.app.version):
            return (hash((self._m_hashTangents, self._m_hashIndex,
                    self._m_hashCoords, self._m_hashNormals, self._m_hashUV,
                    self._m_hashColor)))
        else:
            return (hash((self._m_hashLoopIndex, self._m_hashIndex,
                    self._m_hashCoords, self._m_hashNormals, self._m_hashUV,
                    self._m_hashColor)))

    def __eq__(self, other):
        if (BLENDER_CHECK_VERSION01 <= bpy.app.version):
            return (self._m_hashTangents,  self._m_hashIndex,
                    self._m_hashCoords, self._m_hashNormals, self._m_hashUV,
                    self._m_hashColor) == (other._m_hashTangents,
                                           other._m_hashIndex,
                                           other._m_hashCoords,
                                           other._m_hashNormals,
                                           other._m_hashUV, other._m_hashColor)
        else:
            return (self._m_hashLoopIndex, self._m_hashIndex,
                    self._m_hashCoords, self._m_hashNormals, self._m_hashUV,
                    self._m_hashColor) == (other._m_hashLoopIndex,
                                           other._m_hashIndex,
                                           other._m_hashCoords,
                                           other._m_hashNormals,
                                           other._m_hashUV, other._m_hashColor)

    def _getVertexDataByLoopIndex(self, m_mesh, m_index, m_needNormals,
                                  m_uvLayersIndexes, m_vertColorsIndexes):
        ...
        if ("baked" == self.m_format):  # x z -y
            m_vertexCoords = (m_vertexCoords[0], m_vertexCoords[2],
                              - m_vertexCoords[1])
        ...
        if (m_needNormals):
            ...
            if ("baked" == self.m_format):  # x z -y
                m_vertexNormals = (m_vertexNormals[0], m_vertexNormals[2],
                                   - m_vertexNormals[1])
        ...
        for m_uvlayerIndex in range(m_uvLayersIndexes):
            m_vertexUVs.append(
                m_mesh.uv_layers[m_uvlayerIndex].data[m_index].uv[:])

        for m_vertColorIndex in range(m_vertColorsIndexes):
            m_vertexColors.append(
                m_mesh.vertex_colors[m_vertColorIndex].data[
                    m_vertexIndex].color[:])

        return (m_vertexIndex, m_vertexCoords, m_vertexNormals,
                m_vertexTangents, m_vertexUVs, m_vertexColors)
